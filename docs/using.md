...
host. The gateway sends Pi's harness default (or a replacement system
prompt), then a short platform prompt, then `agent.instructions` when
those are set. See [config](config.md#pi).
